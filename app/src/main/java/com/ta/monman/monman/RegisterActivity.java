...
//    @BindView(R.id.editTextPassword) EditText editTextPassword;
    ...
    private void registerUser() {
        ...
        final String nama_lengkap = editTextName.getText().toString().trim();

        final String gender = ((RadioButton) findViewById(radioGroupGender.getCheckedRadioButtonId())).getText().toString();

        //first we will do the validations
        if (TextUtils.isEmpty(nama_lengkap)) {
            editTextUsername.setError("Tolong isi nama anda");
            editTextUsername.requestFocus();
            return;
        }

        if (TextUtils.isEmpty(username)) {
            editTextUsername.setError("Silahkan isi username");
            editTextUsername.requestFocus();
            return;
        }

        if (TextUtils.isEmpty(email)) {
            editTextEmail.setError("Silahkan isi email dulu");
            editTextEmail.requestFocus();
            return;
        }

        if (!android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            editTextEmail.setError("Format email salah gan");
            editTextEmail.requestFocus();
            return;
        }

        if (TextUtils.isEmpty(password)) {
            editTextPassword.setError("Passwordnya belum diisi coy");
